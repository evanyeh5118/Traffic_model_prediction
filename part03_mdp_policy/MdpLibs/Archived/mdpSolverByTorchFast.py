...
class MdpSolverStochasticByTorch:
    def __init__(self, params):
        ...
        self.lagrange_lambda = torch.tensor(lagrange_lambda, dtype=torch.float32)
    ...
    def initialize(self):
        ...
        self.E_torch = torch.empty(self.N_markov, self.N_Ax, self.N_Ay, dtype=torch.float32)
        ...
        self.T_torch = torch.tensor(self.mdpKernel.getMdpTrans, dtype=torch.float32)
        self.obv_torch = torch.tensor(self.mdpKernel.getMdpObv, dtype=torch.float32)
    ...
    def computeJ(self, policy_x, policy_y, pi_0=None):
        ...
        V = V_r + self.lagrange_lambda*V_c
        J_total = pi_0 @ V[0]
        J_r = pi_0 @ V_r[0]
        J_c = pi_0 @ V_c[0]
    ...
    def UpdateRewardValueTable(self, policy_x, policy_y):
        ...
        Phi = torch.zeros(self.LEN_horizon, self.N_markov, self.N_Ax, self.N_Ay, dtype=torch.float32)
    ...
    def UpdateCostValueTable(self, policy_x, policy_y):
        ...
        Phi = torch.zeros(self.LEN_horizon, self.N_markov, self.N_Ax, self.N_Ay, dtype=torch.float32)
        T_4d = self.T_torch.permute(0, 2, 3, 1)  # shape: (N_markov, N_Ax, N_Ay, N_markov)
        x_range = torch.arange(self.N_Ax, dtype=torch.float32)
        y_range = torch.arange(self.N_Ay, dtype=torch.float32)
        for t in range(self.LEN_horizon-1, -1, -1):
...
class MdpTorchKernel(nn.Module):
    ...
    def forward(self, pi_0=None):
        ...
        J_total, J_r, J_c = self.mdpSolver.computeJ(policy_x, policy_y, pi_0)
        return J_total, J_r, J_c
